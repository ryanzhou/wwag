...
def equipment_create():
  ...
  if request.method == "POST" and form.validate():
    lastrowid = database.execute("INSERT INTO Equipment (ModelAndMake, EquipmentReview, ProcessorSpeed) VALUES (%s, %s, %s);", (form.model.data, form.review.data, form.speed.data)).lastrowid
    database.commit()
...
def equipment_update(equipment_id):
  ...
  form = forms.EquipmentForm(request.form, model=equipment['ModelAndMake'], review=equipment['EquipmentReview'], speed=equipment['ProcessorSpeed'])
  if request.method == "POST" and form.validate():
    database.execute("UPDATE Equipment SET ModelAndMake = %s, EquipmentReview = %s, ProcessorSpeed = %s WHERE EquipmentID = %s;", (form.model.data, form.review.data, form.speed.data, equipment_id))
    database.commit()
